...
colstemp=[i.replace("Capacity(L)",'Capacity(L) /Loading KG') for i in colstemp]
df.columns=colstemp
...
dftype1,dfprice1,dfcap1,dfwidth1,dfdepth1,dfSubcate1,dfWater1,dfSets1=st.tabs(['Type','price','Capacity(L) /Loading KG','Width(cm)','Depth(cm)','DW Subcategory','Water consumption','Sets'])
...
with dftype1:
    ...
    for chose in choses:
        ...
        layout = go.Layout(title="size, growth & share_"+ chose,height=700)
        fig = go.Figure(data=[table_data],layout=layout)
        st.plotly_chart(fig, use_container_width=True)
...
chosex2 = st.radio('请选择交叉分析的变量2（列）？', ['Type','caplabel','widthlabel','depthlabel','DW Subcategory','Setslabel','Waterlabel'],index=0,horizontal=True,key='variablesx2')
